...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='sfbot_description').find('sfbot_description')
    default_model_path = os.path.join(pkg_share, 'urdf/sfbot.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/sfbot/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'sfbot', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
