...
# 모터 제어 명령 저장을 위한 구조 수정
motor_commands = {
    'mode': 'idle',  # 'idle', 'speed', 'position' 중 하나
    'target_motor': None,  # 제어할 모터 ID
    'target_speed': 0,
    'target_position': 0,
...
def set_speed_loop_mode(can_id, speed):
    ...
    motor_commands['target_motor'] = can_id
...
def set_motor_idle(can_id=None):
    """
    모터를 idle 상태로 설정
    Args:
        can_id: 특정 모터 ID. None이면 모든 모터 idle
    """
    global motor_commands
...
def command_sender_thread():
    ...
    while not stop_thread:
        if motor_commands['target_motor'] is None:
            time.sleep(0.1)
            continue
            
        if motor_commands['mode'] == 'speed':
            set_speed_loop_mode(
                motor_commands['target_motor'], 
...
def initialize_motor_origin(motor_id):
    """
    모터의 원점을 설정하는 함수
    Args:
        motor_id: 원점을 설정할 모터 ID (1 또는 2)
    Returns:
        bool: 원점 설정 성공 여부
    """
    ...
    try:
        ...
        print(f"[INFO] Starting origin initialization for motor {motor_id}")
        ...
        # 2. 저속으로 원점 방향으로 이동
        set_speed_loop_mode(motor_id, -100)  # 저속으로 역방향 회전
        
        # 3. 원점 감지 대기 (전류값 모니터링)
        timeout = time.time() + 20  # 20초 타임아웃
        while time.time() < timeout:
            if motor_currents[motor_id-1] > 1.0:  # 전류 임계값 감지
                print(f"[INFO] Origin detected for motor {motor_id}, Current: {motor_currents[motor_id-1]}")
                set_speed_loop_mode(motor_id, 0)  # 모터 정지
                set_origin_mode(motor_id, 0)  # 원점 위치 저장
                set_position_speed_loop(motor_id, 0, 50, 50)
                motor_origin_set[motor_id-1] = True
                return True
            time.sleep(0.1)
            
        print(f"[ERROR] Origin initialization timeout for motor {motor_id}")
        return False
        
    except Exception as e:
        print(f"[ERROR] Origin initialization failed for motor {motor_id}: {e}")
        return False
...
def main():
    ...
    try:
        ...
        if not check_motor_communication():
            ...
            raise Exception("Motor zX communication error")
        print("Motor communication check completed")

        # 1번 모터 원점 초기화
        print("Initializing motor 1 origin...")
        if not initialize_motor_origin(1):
            print("[ERROR] Motor 1 origin initialization failed")
            raise Exception("Motor 1 origin initialization error")
        print("Motor 1 origin initialization completed")
        set_position_speed_loop(1,-90,500,500)
        time.sleep(3)  # 1번 모터 원점 설정 완료 후 안정화 시간

        # 1번 모터 성공 후 2번 모터 원점 초기화
        
        print("Initializing motor 2 origin...")
        if not initialize_motor_origin(2):
            print("[ERROR] Motor 2 origin initialization failed")
            raise Exception("Motor 2 origin initialization error")
        print("Motor 2 origin initialization completed")
        set_position_speed_loop(2,90,500,500)
